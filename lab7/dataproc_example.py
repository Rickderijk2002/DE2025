...
df = spark.read \
  .format("bigquery") \
  .load("de2025-471807.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "temp_de_2025"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'de2025-471807.labdataset.wordcounts') \
  .mode("append") \
  .save()
